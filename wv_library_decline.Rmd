...
#Load packages and set-up. Also load your first dataframe.
```{r}
#Loading packages
library(tidyverse)
library(stringr)
...
#Creating aggregate data  showing percent change per county, not per library 
```{r}
#Creating this data 
...
#Creating a shapefile & combining it with our library finance dataframe to help us make an interactive map. 
```{r}
...
#Creating the actual map. We've also filtered for certain values so the map only colors in counties that have experienced decreases. 
```{r}
...
#Creating the map 
wv_interactive_library_map2 <- leaflet() %>% addProviderTiles("CartoDB.DarkMatterNoLabels") %>% setView(-79.20,38.90,zoom =6.5) %>% addPolygons(data = wv_map_2,fillColor =~pal2(wv_map_2$pct_change),fillOpacity = 10,weight = .6,smoothFactor=.4,popup =~ popup_wv_library2) %>% addLegend(pal=pal2,values=c(0,114),position = "bottomright",title = "% increase in funding") 
